    ...
    projects: [
      {
        ...
        impact:
          "CARLA × Alpamayo 실시간 연동 — 추론 지연 1.2s → 0.78s, FPS 18 → 28, ADE 0.72m / FDE 1.84m 정량 평가 체계 확립",
        description:
          "자율주행 궤적 예측 모델(Alpamayo)을 CARLA 시뮬레이터에 연동해 추론 결과를 실시간 시각화하고, 동시에 정량 지표 기반의 검증·로깅 파이프라인을 구축. 기존 오프라인 검증 방식을 시뮬레이션 루프 내 실시간 검증으로 전환하고, 시나리오별 평가 결과를 자동 로깅해 모델 회귀 검증 사이클을 표준화.",
        tasks: [
        ...
        achievements: [
          "ADE 0.72m, FDE 1.84m, RMSE 0.91m 기준으로 궤적 예측 정확도 수치화",
          "추론 지연 1.2s → 0.78s(약 35%) 단축으로 실시간 검증 안정성 확보",
